...
1. 代码编写、场景节点修改或配置更新完成后，在Act模式下允许继续执行静态检查、启动检查、日志检查等可自动确定结果的技术验证；禁止进行需要连续操作、动态视觉判断、游戏手感或复杂交互才能得出结论的动态交互体验验收
2. 需要人类进行动态交互体验验收时直接向人类开发者输出：“开发任务已完成，请在Godot引擎中进行人工验收”，挂起当前任务，等待人类输入引擎内的实际运行结果日志或视觉反馈
...
2. 必要时运行项目，确认能否正常启动
3. 检查 Editor/Game 日志
4. 可检查无需人工操作即可判断的静态截图
5. 发现技术错误则继续修复并重新验证
6. 涉及连续操作、游戏手感、动态视觉、复杂交互、时序体验等内容时停止验收，交由人类开发者进行人工动态体验验收
